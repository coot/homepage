\documentclass[tikz,multi={tikzpicture},png={outfile=ping-pong.png,density=1000}]{standalone}
...
\newcommand{\MsgPongDoneX}{\tiny\texttt{MsgPongDone2}}
...
\begin{tikzpicture}[->,>=stealth',shorten >=1pt,auto,node distance=1.75cm]
  ...
  \draw (Busy) edge[right, bend left] node{\MsgPongDoneX} (Idle);
  \draw (Busy.120) to [out=120,in=60,looseness=10] node[above]{\MsgPongX} (Busy.60);
\end{tikzpicture}
