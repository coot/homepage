...
![normal form of a morphism in Cat f](/images/free-cat-morphism.png)
...
from `f :: Type -> Type -> Type` to a category `Category g => g :: Type ->
Type -> Type` you can construct (in a unique way) a functor from `Cat f` to
`g`.  This is the spirit of free algebras.  And indeed we can get a `foldMap`
like map:
...
![category (Cat (Single e v)) with one object v](/images/one-object-cat.png)
