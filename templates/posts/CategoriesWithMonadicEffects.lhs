...
identity morphisms and lacks composition, the `Cat` constructurs fill in the
...
composition.  The first one is obvous, since this is how we defined
`foldNatFree2`, so we are only left with the former claim.  We need to consider
two cases, following the defition on composition in `Cat f`.
...
a context of categorical composition.   The following class is an attempt to
give access to both:
...
foldNatFree2 nat (lift mcab)
  -- by defintition of `lift` in `FreeEffCat`
  = foldNatFree2 nat (Lift mcab)
  -- by defintition of `fodlNatFree2`
...
by the type system, to have access to the secret only after a sucessful login.
